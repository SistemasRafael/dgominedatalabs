<?php
    ...
    //$dest = 'absorcion'.'/ '; //PRODUCCION
    $dest = dirname(__DIR__).'\\dgominedatalabs\\absorcion'.'\\ '; // LOCAL

    $desti = rtrim($dest).strtoupper($archivo);
    $u_id_lee = $_SESSION['u_id'];

    $trn_id_bat = $mysqli->query("SELECT 
                                    trn_id_rel,
                                    metodo_id,
                                    (CASE WHEN folio like '%RE%' THEN 1 ELSE 0 END) AS reensaye
    ...
    if( $met_id_b == 14 or $met_id_b == 15 or $met_id_b == 16) {
       for ($indiceFila = 1; $indiceFila <= $numeroMayorDeFila; $indiceFila++) {
        ...
        $import = $mysqli->query("SELECT COUNT(*) AS existe FROM arg_ordenes_csv_detalle WHERE trn_id_rel = ".$trn_id_batch." AND metodo_id = ".$met_id_b) or die(mysqli_error($mysqli));
        ...
        $import = $mysqli->query("SELECT COUNT(*) AS existe FROM arg_ordenes_csv_detalle WHERE trn_id_rel = ".$trn_id_batch) or die(mysqli_error($mysqli));
        $importac = $import->fetch_assoc();
        $archivo_importado = $importac['existe'];
