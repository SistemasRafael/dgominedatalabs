...
if (is_null($fecha_i)){
    $fecha_i = date('Y-m-d',strtotime("-20 days"));//date('d/m/y');
}
if (is_null($fecha_f)){
    $fecha_f = date('Y-m-d');
}
...
<div class="container-fluid">
    ...
    <div class="col-md-1 col-lg-1">
        ...
        <select name="metodo_id_sel" id="metodo_id_sel" class="form-control">
            <? $result_h = $mysqli->query("SELECT metodo_id, nombre FROM `arg_metodos` WHERE metodo_id iN (2,9,30)") or die(mysqli_error($mysqli));
    ...
    <div class="col-md-2 col-lg-2">
        ...
        <select name="tipo_id_sel" id="tipo_id_sel" class="form-control">
            <? $result_h = $mysqli->query("SELECT 1 AS tipo_id, 'Muestras de Geología y Controles de Calidad' AS nombre 
                                                          UNION ALL 
                                                          SELECT 0 AS tipo_id, 'Muestras Geo' AS nombre") or die(mysqli_error($mysqli));
            while ($row2 = $result_h->fetch_array(MYSQLI_ASSOC)) {
                $met_sele = $row2['nombre'];
                $stringt = "";
    ...
    <div>
       ...
       // echo $fecha_i; echo $fecha_f;
        $query = "CALL arg_rpt_pesajesMuestrasMetalurgia(0, 1, 0, '".$fecha_i."','".$fecha_f."')";
        ...
        $html_det = "<table class='table table-striped' id='motivos'>
                                <thead>
                                    <tr class='table-info' justify-content: center;>
                                        ...
                                        <th scope='col1'>Prom Au_gton</th>
                                        <th scope='col1'>Prom Ag_gton</th>
                                        
                                        <th scope='col1'>Peso Húm</th>
                                        <th scope='col1'>Peso Seco</th>
                                        <th scope='col1'>% Húm</th>";
        $html_det .= "</tr>
                               </thead>
                               <tbody>";
                               
                               //   <th scope='col1'>INCUARTE</th>   <th scope='col1'>Ag_PPM</th>
...
<script>
    ...
    function exportar_listado() {
      ...
      //  alert(fecha_f);
        var exportar = '<?php echo "\ exportar_pesajes_muestrasMetalurgia.php?metodo_sel=" ?>' + metodo_sel + '&tipo_mues=' + tipo_mues + '&orden=' + orden + '&fecha_i=' + fecha_i+ '&fecha_f=' + fecha_f;
    ...
    function llama_datos() {
       ...
       // alert(tipo_mues);
        $.ajax({
                url: 'reporte_pesajes_muestrasMet.php',
                type: 'POST',
                dataType: 'html',
                data: {
