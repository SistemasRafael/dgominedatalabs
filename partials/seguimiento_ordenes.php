<?php
    ...
    $fecha_i = $_GET['fecha_i'] ?? date('Y-m-d',strtotime("-30 days"));
    $fecha_f = $_GET['fecha_f'] ?? date('Y-m-d');
...
<script>
  ...
  function iniciar_etapa(trn_id, etapa, unidad)
   ...
   function iniciar_etapa_reen(trn_id, etapa, unidad)
    ...
    function peso_guardar(trn_bat_sec, trn_rel_sec, cont_sec)
    {
        ...
        if(peso_seco == 0 || peso_seco == '') {
            ...
            $.ajax({
                ...
                dataType: 'html',
                data: { 
                    trn_id_sec : trn_id_sec, 
                    trn_id_rel_sec : trn_id_rel_sec, 
                    peso_seco : peso_seco, 
                    fin : fin 
                },
            }).done(function(respuesta){
    ...
    //Metodos quebradora, carbones, metalurgia
    function iniciar_metodo_quebr(trn_id, metodo, fase, etapa, unidad)
    {
            ...
            if (etapa == 30){//Dig Acida CB_DIGAg 9
                $('#metodo_modal').modal('show');
                $("#datos_metodo").html(respuesta);
                $('#metodo_modal').on('shown.bs.modal', function (e) {
                    $(this).find('#hora_inicio').focus();
            ...
            if (etapa == 31){//Filtro muestras CB_DIGAg 9
                $('#metodo_modal').modal('show');
                $("#datos_metodo").html(respuesta);
                $('#metodo_modal').on('shown.bs.modal', function (e) {
                    $(this).find('#hora_inicio').focus();
    ...
    function exportar_absorcion(trn_id_abs, metodo_abs, u_id_exp)
            {
                 ...
                 if(metodo_abs == 33 || metodo_abs == 27 || metodo_abs == 28 || metodo_abs == 9 || metodo_abs == 35){
    ...
    /******** ENVIAR RESULTADOS PARA CARBONES Y METALURGIA *************/
    ...
    function agitacion_guardar(trn_id_ci, metodo_ci, fase_ci, etapa_ci)
        {               
            var trn_id_agi    = trn_id_ci;
            var metodo_agi    = metodo_ci;
            var hora_ini  = document.getElementById("hora_inicio").value;
            var hora_fin  = document.getElementById("hora_final").value;
            var fase_id_agi = fase_ci;
            var etapa_id_agi = etapa_ci;
...
<div class="modal fade" data-backdrop="static" data-keyboard="false" id="importar_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body"  style="font-size:4px;" class="col-md-12 col-lg-12" id="datos_importar"></div>
            <?php
                $action = $_POST["action"] ?? '';
                  ...
                  $html_det = "<table class='table table-striped' id='motivos'>
                                <thead>                                
                                     <tr class='table-info'>      
                                        ...
                                        <th colspan='4' center>Listado</th>";
                               ...
                               foreach ($listDatosOrdenDetalle as $fila) {
                                    ...
                                    if ($fila['estado_id'] == 0) {
                                        if ($fila['humedad'] <> 0) {
                                            ...
                                            $html_det.="><span class='fa fa-check-circle-o fa-2x'>Preparar</span></a></td>";
                                        }                                        
                                    }
                                            ...
                                            foreach (array_filter($Metodoslista, fn($obj) => $obj["trn_id_batch"] == $fila['trn_id_batch']) as $fila_met) {
                                                ...
                                                if ($fila_met['boton_acceso'] <> 0) {                                                                       
                                                    if($fila['orden_sobrelim'] == 1 || $fila_met['metodo_id'] == 1) {
                                                        ...
                                                        if($fila['trn_id_batch'] ==9703 or $fila['trn_id_batch'] ==9704){
                                                            var_dump($fila['trn_id_batch'].",".$fila_met['metodo_id'].", ".$fila_met['fase_id'].", ".$fila_met['etapa_id']);
                                                        }
                                                        $html_det.="onclick = iniciar_metodo(".$fila['trn_id_batch'].",".$fila_met['metodo_id'].",".$fila_met['fase_id'].",".$fila_met['etapa_id'].",".$unidad_id.")";
                                                    }
                                                }

                                                $html_det.="><span class='".$variable_img."'>".$fila_met['nombre']."  ".$fila_met['etapa']." </span></button>";                                                                  
                                            }
                                    ...
                                    if ($fila['prepara'] > 0){       
                                        ...
                                        $html_det.="<td> <a button type='button'class='btn btn-warning' href='etiquetas_muestras_metal.php?trn_id=".$fila['trn_id_batch']."&metodo_id=".$fila['metodo_id']."&tipoMuestras=0' target='_blank'>
                                                            <span class='fa fa-tags fa-2x'></span>
                                                        </button></td>";
                                        if($fila['metalurgia'] == 3){                                                   
                                            $html_det.="<td> <a button type='button'class='btn btn-secondary' href='etiquetas_muestras_metal.php?trn_id=".$fila['trn_id_batch']."&metodo_id=".$fila['metodo_id']."&tipoMuestras=1' target='_blank'>
                                                                        <span class='fa fa-tags fa-2x'></span>
                                                                    </button></td>";
                                        }
                                        else{                                                   
