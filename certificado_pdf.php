...
if (isset($_GET['trn_id'])) {
    ...
    $datos_orden = $mysqli->query("SELECT
                                    ...
                                    LEFT JOIN arg_ordenes_bitacora_detalle AS odb ON od.trn_id = odb.trn_id_rel                                          
                                    WHERE od.trn_id =".$trn_id." AND odb.etapa_id = 12 ORDER BY odb.fecha DESC LIMIT 1"
    ...
    if ($etapa >= 11) {
        $query2 = $mysqli->query("SELECT u.nombre As nombre, u.email As correo, ud.firma as firma 
                                    ...
                                    WHERE ud.u_id =".$uid )or  die(mysqli_error($mysqli)); 
    ...
    $result = $mysqli->query("CALL arg_rpt_generarPDF (".$trn_id.")");
    $pdf_html .= "<div style='margin-top:50px'></div>
                <table align='center'; border='1'; cellspacing='0'> 
                    <thead> 
                        <tr> 
                            <th align='center' colspan='7'> Resultados </th> 
                        </tr> 
                    </thead> 
                    <tbody border = '1'>";
    ...
    while($row = $result->fetch_assoc()) {
        ...
        $field5name = $row["resultado4"];
        $field6name = $row["resultado5"];                                      
        $field7name = $row["resultado6"];

        if ($field2name >= 1 and $cont > 4){
            $field2name = '>1';
        }
        
        $tabla ="<tr cellspacing='0' border = '1'>
                <td> ".$field1name."</td>                                                
                <td>".$field2name."</td>
                <td>".$field3name."</td>
                <td>".$field4name."</td>                                                
