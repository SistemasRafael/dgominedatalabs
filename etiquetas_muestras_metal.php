...
$tipoMuestras = $_GET['tipoMuestras'];
...
if($tipoMuestras == 0){
    $detalle_etiquetas = $mysqli->query("SELECT
                                        od.trn_id_batch,
                                        od.posicion,
                                        od.folio_interno AS muestra_geologia                                      
                                      FROM
                                        ordenes_metalurgia od
                                       WHERE
                                            od.tipo_id = 0
...
while ($page <= $total_paginas) {
    ...
    while ($fila < $total_filas) {
        $fila++;
       if ($fila < 6){
            $html_det .= "<div class='row' style='margin-left: 2%; padding-top:0px;  height: 95px'>";   
        }
        else{
            if ($fila == 7 || $fila == 8){
                $html_det .= "<div class='row' style='margin-left: 2%; padding-top:0px;  height: 105px'>";   
            }
            else{
                $html_det .= "<div class='row' style='margin-left: 2%; height: 100px'>";
            }
        }
        ...
        while ($columna < 4 && $i <= $total_muestras) {
            ...
            if ($columna == 1) {
                $html_det .= "<div class='column' style='right:100%;  padding-left:0px;'>
            ...
            if ($columna == 2 && $i <= $total_muestras) {
                $html_det .= "<div class='column' style='right:100%;  padding-left:22px;'>
                          <table border='1' cellspacing='0'  style='font-size: 70%;'>
                          <tbody>   
                                <tr>
                                    <th align='center' colspan='3'>Unidad " . $mina . "</th> 
