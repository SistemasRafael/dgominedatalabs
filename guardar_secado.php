<?php
    ...
    if (isset($trn_id)){
        ...
        $result = $mysqli->query("SELECT folio_interno AS orden_trabajo 
                                    FROM arg_ordenes_detalle  
                                    WHERE trn_id = ".$trn_id) or die(mysqli_error($mysqli));
        ...
        if(str_contains($orden_trabajo, '-STK')) {
            $resultado = $mysqli->query("SELECT 
                                            ...
                                            ,se.peso
                                            ,mm.folio as muestra 
                                        FROM arg_muestras_secado se
                                            LEFT JOIN arg_ordenes_muestrasMetalurgia mm
                                            ON mm.trn_id_rel = se.trn_id 
                                            AND mm.trn_id = se.trn_id_rel
                                            WHERE se.trn_id = ".$trn_id." AND se.peso = 0") or die(mysqli_error($mysqli));
        } 
        else {
            $resultado = $mysqli->query("SELECT
                                                se.trn_id as trn_id_batch
